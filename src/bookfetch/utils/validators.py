...
def validate_archive_url(url: str) -> bool:
    """Validate if URL is a valid Archive.org details URL.
    ...
    if not url.startswith("https://archive.org/details/"):
        raise ValidationError(
            f"Invalid URL: {url}. URL must start with 'https://archive.org/details/'"
        )
    return True
...
def extract_book_id(url: str) -> str:
    """Extract book ID from Archive.org URL.
    ...
    Raises:
        ValidationError: If book ID cannot be extracted
    """
    try:
        validate_archive_url(url)
        ...
        raise ValidationError(f"Cannot extract book ID from URL {url}: {e}")
