...
def login(email, password):
    ...
    data = format_data(
        content_type, {"username": email, "password": password, "submit_by_js": "true"}
    )
...
if __name__ == "__main__":
    ...
    my_parser.add_argument(
        "-f", "--file", help="File where are stored the URLs of the books to download", type=str
    )
    my_parser.add_argument(
        "-r",
        "--resolution",
    ...
    my_parser.add_argument(
        "-t", "--threads", help="Maximum number of threads, [default 50]", type=int, default=50
    )
    my_parser.add_argument(
        "-j", "--jpg", help="Output to individual JPG's rather than a PDF", action="store_true"
    )
    my_parser.add_argument(
        "-m",
        "--meta",
